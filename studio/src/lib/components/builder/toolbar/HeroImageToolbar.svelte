<script lang="ts">
    ...
    import { selectedHeroImageNode, updateNode } from '$lib/stores/momentContent'
    import { registerImage } from '$lib/utils/imageUpload'

    let fileInput: HTMLInputElement

    const handleFileSelected = (e: Event) => {
        if (!$selectedHeroImageNode) return
        const file = (e.target as HTMLInputElement).files?.[0]
        if (!file) return
        updateNode($selectedHeroImageNode.id, { src: registerImage(file) });
